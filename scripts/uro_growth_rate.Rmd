...
We had to clean parts of the data to prepare it for analysis. The first issue we had to resolve was the extreme weight and caliper length outliers, which was clearly due to a misplaced decimal or missing 0 during data entry. In the silenced part of the markdown, this had to be corrected 5/432 times. 

The second issue was that some snails ran out of food during the growth experiment, which could jeopardize our assumption of unlimited growth while in the common garden experiment. The vast majority (391/432) of snails never ran out of food. 40/432 ran out of food once, while 1/432 ran out of food twice. For the purposes of this experiment, we decided to include snails that missed a single meal during the entirity of the experiment, but removed the single case in which a snail ran out food twice. 

The third issue was that some snails died, or went missing or were crushed, over the course of the experiment. We removed these snails from consideration at the timepoint they died or went missing. 
...
ggplot(data=temp,aes(x=date,y=WTMP,color=site))+scale_color_viridis_d(name="Site",labels=c("Great Bay, NH","Woods Hole, MA","Oyster, VA","Beaufort, NC","Folly Beach, SC","Skidaway, GA","Willapa, WA","Humboldt, CA"),option="D")+theme_classic()+geom_vline(xintercept = 152)+geom_vline(xintercept = 273)+ylab("SST (°C)")+xlab("")+scale_fill_discrete(name="Site",labels=c("Great Bay, NH","Woods Hole, MA","Oyster, VA","Beaufort, NC","Folly Beach, SC","Skidaway, GA","Willapa, WA","Humboldt, CA"))+scale_x_date(date_labels = "%b")+theme(text=element_text(family="sanserif",size=14))+stat_smooth(se=,size=2)+geom_point(alpha=0.0002,color="black",fill="white")+guides(color=guide_legend(override.aes=list(fill=NA)))+geom_hline(yintercept=10)
...
Do populations differ in their growth rate? Here, ANOVA tells us that growth betwween sites are signficantly different. 
...
Should we use end caliper lengths, or do we need to subtract initial caliper length from end caliper length? In other words, do initial caliper lengths differ, requiring us to standardize our growth rate? Here, we find that populations do differ in initial growth. Thus, we must standardize growth by creating a growth rate of Final size - initial size. The univariate boxplots below also show that while outliers do appear to be present, they can be attributed to population or temperature level differences. 
```{r,echo=F}
...
Here, we investigate the residuals of the full model. 

```{r,echo=F,warning=F}
growth.alive<-na.omit(growth.alive)
fulli<-(lm(cal.length~pop*temp*oce*bin,growth.alive))

plot(resid(fulli))
hist(resid(fulli))
skewness(growth.alive$cal.length)
#negative skew
growth.alive<-tidyr::drop_na(growth.alive)
growth.alive<-na.omit(growth.alive)
...
```
While the log transformation improves the skewness slightly (silenced but in code), we will assume the distribution of residuals are normal and proceed with untransformed growth rate. 

## Coplots

Other than temperature and population, which we control for, do we see different reactions depending on bin? Here, reactions appaer to be the same no matter what bin we used. Note: Margins were too big on this figure to include in the markdown - viewable in R console. 
```{r,echo=F}
#dev.off()
...
```
Here, we see that a few models are well supported. We choose the interactive pop*temp model only because 1) the additive model only tells us if populations are different at each temperature, while the additive model also tells us the populations slopes with temperature and gives us a TPC 2) oce adds nothing to the models, so is removed. 3) Looking at our coplots of bin, bin had no effect on growth. 
...
seg.gb<-segmented(m.gb,seg.Z = ~temp, psi=28,seg.control(maxit.glm=100),fix.npsi=T,n.boot=500)
...
#+geom_boxplot(data=growth.alive,aes(x=temp,y=cal.length,group=interaction(temp,pop),fill=pop))
...
```{r,echo=F,warning=F}
scores<-data.frame("site" = c("Great Bay","Woods Hole","Oyster","Beaufort","Folly Beach","Skidaway","Willapa","Humboldt"),"P Score"=NA,"Davies Test"=NA,"CI Low"=NA,"CI High"=NA,"breakpoint"=NA)
scores[1,2]<-pscore.test(seg.gb,~temp)[[5]]
scores[2,2]<-pscore.test(seg.wh,~temp)[[5]]
scores[3,2]<-pscore.test(seg.oy,~temp)[[5]]
scores[4,2]<-pscore.test(seg.bf,~temp)[[5]]
scores[5,2]<-pscore.test(seg.fb,~temp)[[5]]
scores[6,2]<-pscore.test(seg.sk,~temp)[[5]]
scores[7,2]<-pscore.test(seg.wp,~temp)[[5]]
scores[8,2]<-pscore.test(seg.hm,~temp)[[5]]

scores[1,3]<-davies.test(seg.gb,~temp)[[5]]
scores[2,3]<-davies.test(seg.wh,~temp)[[5]]
scores[3,3]<-davies.test(seg.oy,~temp)[[5]]
scores[4,3]<-davies.test(seg.bf,~temp)[[5]]
scores[5,3]<-davies.test(seg.fb,~temp)[[5]]
scores[6,3]<-davies.test(seg.sk,~temp)[[5]]
scores[7,3]<-davies.test(seg.wp,~temp)[[5]]
scores[8,3]<-davies.test(seg.hm,~temp)[[5]]

scores[1,4]<-confint(seg.gb)[[2]]
scores[2,4]<-confint(seg.wh)[[2]]
scores[3,4]<-confint(seg.oy)[[2]]
scores[4,4]<-confint(seg.bf)[[2]]
scores[5,4]<-confint(seg.fb)[[2]]
scores[6,4]<-confint(seg.sk)[[2]]
scores[7,4]<-confint(seg.wp)[[2]]
scores[8,4]<-confint(seg.hm)[[2]]

scores[1,5]<-confint(seg.gb)[[3]]
scores[2,5]<-confint(seg.wh)[[3]]
scores[3,5]<-confint(seg.oy)[[3]]
scores[4,5]<-confint(seg.bf)[[3]]
scores[5,5]<-confint(seg.fb)[[3]]
scores[6,5]<-confint(seg.sk)[[3]]
scores[7,5]<-confint(seg.wp)[[3]]
scores[8,5]<-confint(seg.hm)[[3]]

scores[1,6]<-confint(seg.gb)[[1]]
scores[2,6]<-confint(seg.wh)[[1]]
scores[3,6]<-confint(seg.oy)[[1]]
scores[4,6]<-confint(seg.bf)[[1]]
scores[5,6]<-confint(seg.fb)[[1]]
scores[6,6]<-confint(seg.sk)[[1]]
scores[7,6]<-confint(seg.wp)[[1]]
scores[8,6]<-confint(seg.hm)[[1]]
...
brkpts$seasonlength16<-as.numeric(brkpts$seasonlength16)
...
Here, it appears that the season length as calculated at 10C is the best predictor. This is followed by latitude and season length calculated at 16C. latitude doesn't tell us much about the environment, so let's take a look at how both season length metrics we calculated perform when calculating maximal trait performance.

```{r,include=T,warning=F,echo=F}
##Maximal trait performance (y)

brkptym<-(lm(brkpty~seasonlength16,brkpts))
summary(brkptym) 


ggplot(brkpts,aes(x=seasonlength16,y=brkpty,color=oce))+
  geom_point(size=3)+theme_classic()+
  ylab("Maximal Growth (mm)")+
  xlab("Season Length (days), 16C")+
...
ggplot(brkpts,aes(x=seasonlength10,y=brkpty,color=oce,fill=oce),group=oce)+
  geom_point(size=3)+theme_classic()+
  ylab("Maximal Growth (mm)")+
...
When we look at season length when calcualted at 10C, we see a signficant relationship between maximal trait performance and season length. This trend disappears when we calcualte it at 16C, mostly because our Pacific sites "ungroup" from the Atlantic sites. At 10C season length, we could be seeing possible countergradient variation! The one weird point in the Atlantic is Woods Hole. Anecdotally, I could attribute this to the very hot local conditions in the estuary, but can't say for sure. 
...
brkptym<-lm(brkptx~seasonlength10,brkpts)
summary(brkptym)

ggplot(brkpts,aes(x=seasonlength10,y=brkptx,color=oce),group=interaction(oce))+
...
mods.brkx2<-list(
  ...
  "seasonlengtho16"=glm(brkptx~seasonlength16+oce,brkpts_wh,family="gaussian"),
  ...
  "seasonlength16"=glm(brkptx~seasonlength16,brkpts_wh,family="gaussian"),
  ...
  "seasonlength16"=glm(brkptx~seasonlength16*oce,brkpts_wh,family="gaussian"),
...
ggplot(brkpts_wh,aes(x=seasonlength10,y=brkptx,color=oce),group=interaction(oce))+
  geom_point(size=3)+geom_smooth(aes(group=oce),method="lm",se=F)+theme_classic()+
  ylab("Thermal Optima")+
  xlab("Season Length (10c)")+
  theme(text=element_text(family="arial",size=22))+
...
ggplot(all.length.pred,aes(x=temp,y=cal.length))+geom_line(aes(group=pop,x=temp,y=cal.length,color=pop,linetype=pop,size=pop))+
  ylab("Shell Length (mm)")+xlab("Common Garden Temperature (°C)")+theme_classic()+
  scale_x_continuous(breaks=c(16,20,24,26,28,30))+
  scale_linetype_manual(values=c(1,1,1,1,1,1,1,1),labels=c("Great Bay","Woods Hole","Oyster","Beaufort","Folly Beach","Skidaway","Willapa","Humboldt"),name="pop")+
  scale_size_manual(values=c(1.2,1.2,1.2,1.2,1.2,1.2,1.2,1.2),labels=c("Great Bay","Woods Hole","Oyster","Beaufort","Folly Beach","Skidaway","Willapa","Humboldt"),name="pop")+
...
brkptxmq<-glm(brkptyq~mean+oce,brkpts,family="gaussian")
summary(brkptxmq)

ggplot(brkpts,aes(x=mean,y=brkptyq,color=oce),group=interaction(oce))+
  geom_point(size=3)+geom_smooth(aes(group=oce),method="lm",se=F)+theme_classic()+
  ylab("Thermal Optima")+
  xlab("Mean (degrees C)")+
  theme(text=element_text(family="arial",size=22))+
  ...
  scale_fill_manual(labels=c("Atlantic","Pacific"),name="Ocean", values=c('red','blue'))
```
This is not signficant in the slightest. part of the issue is that the quadratic optima for Oyster site is near 34C. Very unlikely! This is likely due to the very small breakpoint change in Oyster that is interpreted by the quadratic as a single slope. Basically, nothing predicts it well
...
brkptxmq<-glm(brkptxq~seasonlength10,brkpts,family="gaussian")
summary(brkptxmq)

ggplot(brkpts,aes(x=seasonlength10,y=brkptxq,color=oce),group=interaction(oce))+
  geom_point(size=3)+geom_smooth(aes(group=oce),method="lm",se=F)+theme_classic()+
