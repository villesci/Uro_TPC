...
#eliminate snails who ran out of food. 
...
Common garden temperature the snails were raised in for 24 hrs. Degrees C
...
Should we use end caliper lengths, or do we need to subtract initial caliper length from end caliper length? In other words, do initial caliper lengths differ, requiring us to standardize our growth rate? Here, we find that populations do differ in initial growth. Thus, we must standardize growth by creating a growth rate of Final size - initial size. 
```{r,echo=F}
...
mods.brk<-list(
  "null"=lm(brkpty~1,brkpts),
  "lat"=lm(brkpty~lat+oce,brkpts),
  "mean"=lm(brkpty~mean+oce,brkpts),
  "s.mean"=lm(brkpty~s.mean+oce,brkpts),
  "q.mean"=lm(brkpty~q.mean+oce,brkpts),
  "t.mean"=lm(brkpty~t.mean+oce,brkpts),
  "max"=lm(brkpty~max+oce,brkpts),
  "*lat"=lm(brkpty~lat*oce,brkpts),
  "*mean"=lm(brkpty~mean*oce,brkpts),
  "*s.mean"=lm(brkpty~s.mean*oce,brkpts),
  "*q.mean"=lm(brkpty~q.mean*oce,brkpts),
...
brkptym<-(lm(brkpty~lat+oce,brkpts))
summary(brkptym) 
...
Here, I see that both latitude and ocean significantly describe the trend in maximal trait performance. This means possible countergradient variation! The one weird point in the Atlantic is Woods Hole. Anecdotally, I could attribute this to the very hot local conditions in the estuary, but can't say for sure. 
...
mods.brkx<-list(
  "null"=lm(brkptx~1,brkpts),
  "lat"=lm(brkptx~lat+oce,brkpts),
  "mean"=lm(brkptx~mean+oce,brkpts),
  "s.mean"=lm(brkptx~s.mean+oce,brkpts),
  "q.mean"=lm(brkptx~q.mean+oce,brkpts),
  "t.mean"=lm(brkptx~t.mean+oce,brkpts),
    "max"=lm(brkptx~max+oce,brkpts),
  "*lat"=lm(brkptx~lat*oce,brkpts),
  "*mean"=lm(brkptx~mean*oce,brkpts),
  "*s.mean"=lm(brkptx~s.mean*oce,brkpts),
  "*q.mean"=lm(brkptx~q.mean*oce,brkpts),
...
brkptym<-lm(brkptx~lat+oce,brkpts)
summary(brkptym)
...
Once again, null model is best! Once again, I go with lat as the best predictor. In this case, however, we see that thermal optima does not trend signficantly upward with latitude. I can attribute this insignicance mostly to the presence of the Woods Hole population. If we remove this, the trend becomes slightly more signficant.
...
brkptym_wh<-lm(brkptx~lat+oce,brkpts_wh)
summary(brkptym_wh)

ggplot(brkpts_wh,aes(x=lat,y=brkptx,color=oce),group=interaction(oce))+
  geom_point(size=3)+geom_smooth(aes(group=oce),method="lm",se=F)+theme_classic()+
  ylab("Thermal Optima")+
  xlab("latitude")+
  theme(text=element_text(family="arial",size=22))+
...
aictab(mods.brkyq)
#mean is the most highly supported model this time. However, for replicability sake, let's see what happens when we use latitude
...
brkptxmq<-glm(brkptxq~lat+oce,brkpts,family="gaussian")
summary(brkptxmq)
