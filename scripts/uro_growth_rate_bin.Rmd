...
#wh
m.wh1<-glm(cal.length~temp,wh1,family=gaussian)
seg.wh1<-segmented(m.wh1,seg.Z = ~temp, psi=24,seg.control(maxit.glm=100),fix.npsi=T,n.boot=500)

m.wh2<-glm(cal.length~temp,wh2,family=gaussian)
seg.wh2<-segmented(m.wh2,seg.Z = ~temp, psi=24,seg.control(maxit.glm=100),fix.npsi=T,n.boot=500)

m.wh3<-glm(cal.length~temp,wh3,family=gaussian)
seg.wh3<-segmented(m.wh3,seg.Z = ~temp, psi=24,seg.control(maxit.glm=100),fix.npsi=T,n.boot=500)

#mcp test
library(mcp)
library(ggdist)
model=list(cal.length~1+temp,~0+temp)
fit_gb2<-mcp(model,data=gb2,iter=100000,cores=3,prior=list(cp_1="dunif(20,30)"))
plot(fit_gb2, q_fit = T, q_predict = T)

#rTPC test
library(rTPC)
start_vals<-get_start_vals(oy3$temp,oy3$cal.length,model_name='rezende_2019')
low_lims <- get_lower_lims(oy3$temp, oy3$cal.length, model_name = 'rezende_2019')
upper_lims <- get_upper_lims(oy3$temp, oy3$cal.length, model_name = 'rezende_2019')

mod<-nls_multstart(cal.length~rezende_2019(temp=temp,q10,a,b,c),data=oy3,iter=c(3,3,3,3),
...
# get predictions
preds <- data.frame(temp = seq(min(oy3$temp), max(oy3$temp), length.out = 100))
preds <- broom::augment(mod, newdata = preds)

# plot
ggplot(preds) +
  geom_point(aes(temp, cal.length), oy3) +
  geom_line(aes(temp, .fitted), col = 'blue') +
  theme_bw()

start_vals<-get_start_vals(gb2$temp,gb2$cal.length,model_name='rezende_2019')
low_lims <- get_lower_lims(gb2$temp, gb2$cal.length, model_name = 'rezende_2019')
upper_lims <- get_upper_lims(gb2$temp, gb2$cal.length, model_name = 'rezende_2019')

mod2<-nls_multstart(cal.length~rezende_2019(temp=temp,q10,a,b,c),data=gb2,iter=c(3,3,3,3),
...
# get predictions
preds2 <- data.frame(temp = seq(min(gb2$temp), max(gb2$temp), length.out = 100))
preds2 <- broom::augment(mod2, newdata = preds2)

# plot
ggplot(preds2) +
  geom_point(aes(temp, cal.length), gb2) +
  geom_line(aes(temp, .fitted), col = 'blue') +
  theme_bw()
...
m.gb1<-glm(cal.length~temp,gb1,family=gaussian)
seg.gb1<-segmented(m.gb1,seg.Z = ~temp, psi=29,seg.control(maxit.glm=100),fix.npsi=T,n.boot=500)

m.gb2<-glm(cal.length~temp,gb2,family=gaussian)
seg.gb2<-segmented(m.gb2,seg.Z = ~temp, psi=29.5,seg.control(maxit.glm=100),fix.npsi=T,n.boot=500)

m.gb3<-glm(cal.length~temp,gb3,family=gaussian)
seg.gb3<-segmented(m.gb3,seg.Z = ~temp, psi=29,seg.control(maxit.glm=100),fix.npsi=T,n.boot=500)

#oy

m.oy1<-glm(cal.length~temp,oy1,family=gaussian)
seg.oy1<-segmented(m.oy1,seg.Z = ~temp, psi=29.5,seg.control(maxit.glm=100),fix.npsi=T,n.boot=500)

m.oy2<-glm(cal.length~temp,oy2,family=gaussian)
seg.oy2<-segmented(m.oy2,seg.Z = ~temp, psi=24)

m.oy3<-glm(cal.length~temp,oy3,family=gaussian)
seg.oy3<-segmented(m.oy3,seg.Z = ~temp, psi=29,seg.control(maxit.glm=100),fix.npsi=T,n.boot=500)

#bf

m.bf1<-glm(cal.length~temp,bf1,family=gaussian)
seg.bf1<-segmented(m.bf1,seg.Z = ~temp, psi=24)

m.bf2<-glm(cal.length~temp,bf2,family=gaussian)
seg.bf2<-segmented(m.bf2,seg.Z = ~temp, psi=24)

m.bf3<-glm(cal.length~temp,bf3,family=gaussian)
seg.bf3<-segmented(m.bf3,seg.Z = ~temp, psi=24)

#fb

m.fb1<-glm(cal.length~temp,fb1,family=gaussian)
seg.fb1<-segmented(m.fb1,seg.Z = ~temp, psi=24)

m.fb2<-glm(cal.length~temp,fb2,family=gaussian)
seg.fb2<-segmented(m.fb2,seg.Z = ~temp, psi=24)

m.fb3<-glm(cal.length~temp,fb3,family=gaussian)
seg.fb3<-segmented(m.fb3,seg.Z = ~temp, psi=24)

#sk

m.sk1<-glm(cal.length~temp,sk1,family=gaussian)
seg.sk1<-segmented(m.sk1,seg.Z = ~temp, psi=22,seg.control(maxit.glm=100),fix.npsi=T,n.boot=500)

m.sk2<-glm(cal.length~temp,sk2,family=gaussian)
seg.sk2<-segmented(m.sk2,seg.Z = ~temp, psi=22,seg.control(maxit.glm=100),fix.npsi=T,n.boot=500)

m.sk3<-glm(cal.length~temp,sk3,family=gaussian)
seg.sk3<-segmented(m.sk3,seg.Z = ~temp, psi=22,seg.control(maxit.glm=100),fix.npsi=T,n.boot=500)

#hm

hm<-na.omit(hm)
m.hm1<-glm(cal.length~temp,hm1,family=gaussian)
seg.hm1<-segmented(m.hm1,seg.Z = ~temp, psi=24)

m.hm2<-glm(cal.length~temp,hm2,family=gaussian)
seg.hm2<-segmented(m.hm2,seg.Z = ~temp, psi=24)

m.hm3<-glm(cal.length~temp,hm3,family=gaussian)
seg.hm3<-segmented(m.hm3,seg.Z = ~temp, psi=24)

#wp

m.wp1<-glm(cal.length~temp,wp1,family=gaussian)
seg.wp1<-segmented(m.wp1,seg.Z = ~temp, psi=24)

m.wp2<-glm(cal.length~temp,wp2,family=gaussian)
seg.wp2<-segmented(m.wp2,seg.Z = ~temp, psi=24)

m.wp3<-glm(cal.length~temp,wp3,family=gaussian)
seg.wp3<-segmented(m.wp3,seg.Z = ~temp, psi=24)
...
all.length.seg<-list(seg.gb1,seg.wh1,seg.oy1,seg.bf1,seg.fb1,seg.sk1,seg.wp1,seg.hm1,seg.gb2,seg.wh2,seg.oy2,seg.bf2,seg.fb2,seg.sk2,seg.wp2,seg.hm2,seg.gb3,seg.wh3,seg.oy3,seg.bf3,seg.fb3,seg.sk3,seg.wp3,seg.hm3)


##plotting predicted values
#gb
predicted.gb1<-data.frame(temp=seq(xmin,xmax,length.out=100))
predicted.gb1$cal.length<-predict(seg.gb1,predicted.gb1)
predicted.gb1$pop<-"gb"
predicted.gb1$oce<-"a"
predicted.gb1$bin<-1

predicted.gb2<-data.frame(temp=seq(xmin,xmax,length.out=100))
predicted.gb2$cal.length<-predict(seg.gb2,predicted.gb2)
predicted.gb2$pop<-"gb"
predicted.gb2$oce<-"a"
predicted.gb2$bin<-2


predicted.gb3<-data.frame(temp=seq(xmin,xmax,length.out=100))
predicted.gb3$cal.length<-predict(seg.gb3,predicted.gb3)
predicted.gb3$pop<-"gb"
predicted.gb3$oce<-"a"
predicted.gb3$bin<-3
...
predicted.wh1<-data.frame(temp=seq(xmin,xmax,length.out=100))
predicted.wh1$cal.length<-predict(seg.wh1,predicted.wh1)
predicted.wh1$pop<-"wh"
predicted.wh1$oce<-"a"
predicted.wh1$bin<-1


predicted.wh2<-data.frame(temp=seq(xmin,xmax,length.out=100))
predicted.wh2$cal.length<-predict(seg.wh2,predicted.wh2)
predicted.wh2$pop<-"wh"
predicted.wh2$oce<-"a"
predicted.wh2$bin<-2


predicted.wh3<-data.frame(temp=seq(xmin,xmax,length.out=100))
predicted.wh3$cal.length<-predict(seg.wh3,predicted.wh3)
predicted.wh3$pop<-"wh"
predicted.wh3$oce<-"a"
predicted.wh3$bin<-3
...
predicted.oy1<-data.frame(temp=seq(xmin,xmax,length.out=100))
predicted.oy1$cal.length<-predict(seg.oy1,predicted.oy1)
predicted.oy1$pop<-"oy"
predicted.oy1$oce<-"a"
predicted.oy1$bin<-1


predicted.oy2<-data.frame(temp=seq(xmin,xmax,length.out=100))
predicted.oy2$cal.length<-predict(seg.oy2,predicted.oy2)
predicted.oy2$pop<-"oy"
predicted.oy2$oce<-"a"
predicted.oy2$bin<-2


predicted.oy3<-data.frame(temp=seq(xmin,xmax,length.out=100))
predicted.oy3$cal.length<-predict(seg.oy3,predicted.oy3)
predicted.oy3$pop<-"oy"
predicted.oy3$oce<-"a"
predicted.oy3$bin<-3
...
predicted.bf1<-data.frame(temp=seq(xmin,xmax,length.out=100))
predicted.bf1$cal.length<-predict(seg.bf1,predicted.bf1)
predicted.bf1$pop<-"bf"
predicted.bf1$oce<-"a"
predicted.bf1$bin<-1


predicted.bf2<-data.frame(temp=seq(xmin,xmax,length.out=100))
predicted.bf2$cal.length<-predict(seg.bf2,predicted.bf2)
predicted.bf2$pop<-"bf"
predicted.bf2$oce<-"a"
predicted.bf2$bin<-2


predicted.bf3<-data.frame(temp=seq(xmin,xmax,length.out=100))
predicted.bf3$cal.length<-predict(seg.bf3,predicted.bf3)
predicted.bf3$pop<-"bf"
predicted.bf3$oce<-"a"
predicted.bf3$bin<-3
...
predicted.fb1<-data.frame(temp=seq(xmin,xmax,length.out=100))
predicted.fb1$cal.length<-predict(seg.fb1,predicted.fb1)
predicted.fb1$pop<-"fb"
predicted.fb1$oce<-"a"
predicted.fb1$bin<-1


predicted.fb2<-data.frame(temp=seq(xmin,xmax,length.out=100))
predicted.fb2$cal.length<-predict(seg.fb2,predicted.fb2)
predicted.fb2$pop<-"fb"
predicted.fb2$oce<-"a"
predicted.fb2$bin<-2


predicted.fb3<-data.frame(temp=seq(xmin,xmax,length.out=100))
predicted.fb3$cal.length<-predict(seg.fb3,predicted.fb3)
predicted.fb3$pop<-"fb"
predicted.fb3$oce<-"a"
predicted.fb3$bin<-3
...
predicted.sk1<-data.frame(temp=seq(xmin,xmax,length.out=100))
predicted.sk1$cal.length<-predict(seg.sk1,predicted.sk1)
predicted.sk1$pop<-"sk"
predicted.sk1$oce<-"a"
predicted.sk1$bin<-1


predicted.sk2<-data.frame(temp=seq(xmin,xmax,length.out=100))
predicted.sk2$cal.length<-predict(seg.sk2,predicted.sk2)
predicted.sk2$pop<-"sk"
predicted.sk2$oce<-"a"
predicted.sk2$bin<-2


predicted.sk3<-data.frame(temp=seq(xmin,xmax,length.out=100))
predicted.sk3$cal.length<-predict(seg.sk3,predicted.sk3)
predicted.sk3$pop<-"sk"
predicted.sk3$oce<-"a"
predicted.sk3$bin<-3
...
predicted.wp1<-data.frame(temp=seq(xmin,xmax,length.out=100))
predicted.wp1$cal.length<-predict(seg.wp1,predicted.wp1)
predicted.wp1$pop<-"wp"
predicted.wp1$oce<-"p"
predicted.wp1$bin<-1


predicted.wp2<-data.frame(temp=seq(xmin,xmax,length.out=100))
predicted.wp2$cal.length<-predict(seg.wp2,predicted.wp2)
predicted.wp2$pop<-"wp"
predicted.wp2$oce<-"p"
predicted.wp2$bin<-2

predicted.wp3<-data.frame(temp=seq(xmin,xmax,length.out=100))
predicted.wp3$cal.length<-predict(seg.wp3,predicted.wp3)
predicted.wp3$pop<-"wp"
predicted.wp3$oce<-"p"
predicted.wp3$bin<-3
...
predicted.hm1<-data.frame(temp=seq(xmin,xmax,length.out=100))
predicted.hm1$cal.length<-predict(seg.hm1,predicted.hm1)
predicted.hm1$pop<-"hm"
predicted.hm1$oce<-"p"
predicted.hm1$bin<-1


predicted.hm2<-data.frame(temp=seq(xmin,xmax,length.out=100))
predicted.hm2$cal.length<-predict(seg.hm2,predicted.hm2)
predicted.hm2$pop<-"hm"
predicted.hm2$oce<-"p"
predicted.hm2$bin<-2


predicted.hm3<-data.frame(temp=seq(xmin,xmax,length.out=100))
predicted.hm3$cal.length<-predict(seg.hm3,predicted.hm3)
predicted.hm3$pop<-"hm"
predicted.hm3$oce<-"p"
predicted.hm3$bin<-3
...
all.length.pred<-rbind(predicted.gb1,predicted.wh1,predicted.oy1,predicted.bf1,predicted.fb1,predicted.sk1,predicted.wp1,predicted.hm1,predicted.gb2,predicted.wh2,predicted.oy2,predicted.bf2,predicted.fb2,predicted.sk2,predicted.wp2,predicted.hm2,predicted.gb3,predicted.wh3,predicted.oy3,predicted.bf3,predicted.fb3,predicted.sk3,predicted.wp3,predicted.hm3)
...
ggplot(all.length.pred,aes(x=temp,y=cal.length))+geom_line(aes(group=interaction(population,bin),x=temp,y=cal.length,color=population,linetype=population,size=population),size=1.5)+
...
ggplot()+geom_line(data=predicted.wh,aes(x=temp,y=cal.length,group=pop,color=pop),color="navy",size=1.2)+theme_classic()+
scale_x_continuous(breaks=c(16,20,24,26,28,30))+
scale_y_continuous(breaks=c(1,2,3,4,5),labels=c(1,2,3,4,5),limits=c(1,5))+geom_point(data=wh,aes(x=temp,y=cal.length,group="pop",fill="Woods Hole, MA"),color="black")+ ylab("growth rate (mm)")+xlab("temperature (°C)")+theme(text=element_text(family="Times",size=22,color="black"),axis.text=element_text(color="black",size=22))+geom_segment(aes(x=24.82297, y=1,xend=24.82297,yend=4.221764),size=2,linetype=2,color="gold2")+geom_segment(aes(x=16, y=4.221764,xend=24.82297,yend=4.221764),size=2,linetype=2,color="gold2")+annotate("point",x=24.82297,y=4.221764,color="red",size=6)
