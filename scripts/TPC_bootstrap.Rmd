...
```{r,include=F}
growth.alive<-read.csv(here::here("data/growth.csv"))
growth.alive<-na.omit(growth.alive)
growth.alive<-data.frame("temp"=growth.alive$temp,"cal.length"=growth.alive$cal.length,"bin"=growth.alive$bin,"pop"=growth.alive$pop)
...
```{r,echo=F,warning=F}
# fit  rezende model
d_fit_all <- nest(growth.alive, data = c(temp, cal.length)) %>%
...
d_fit <- nest(gb_t, data = c(temp, cal.length)) %>%
  mutate(rezende = map(data, ~nls_multstart(cal.length~rezende_2019(temp = temp, q10,a,b,c),
                                                     data = .x,
                                                     iter = c(3,3,3,3),
                                                     start_lower = get_start_vals(.x$temp, .x$cal.length, model_name = 'rezende_2019') - 1,
...
d_fit_boot <- nest(gb_t, data = c(temp, cal.length)) %>%
  mutate(rezende = map(data, ~nls_multstart(cal.length~rezende_2019(temp = temp, q10,a,b,c),
                                                     data = .x,
                                                     iter = c(3,3,3,3),
                                                     start_lower = get_start_vals(.x$temp, .x$cal.length, model_name = 'rezende_2019') - 1,
...
scale_y_continuous(breaks=c(1,2,3,4,5,6),labels=c(1,2,3,4,5,6),limits=c(1,6))+geom_point(data=gb_t,aes(x=temp,y=cal.length,group="pop"),color="black",size=3)+ ylab("growth rate (mm)")+xlab("temperature (°C)")+theme(text=element_text(family="Times",size=22,color="black"),axis.text=element_text(color="black",size=22),legend.title = element_blank())+geom_segment(aes(x=28.06, y=1,xend=28.06,yend=5.06),size=2,linetype=2,color="gold2")+geom_segment(aes(x=16, y=5.06,xend=28.06,yend=5.06),size=2,linetype=2,color="gold2")+annotate("point",x=28.06,y=5.06,color="red",size=6)
...
for (i in 1:length(df)){
  d_fit_i<-nest(df[[i]], data = c(temp, cal.length)) %>%
  mutate(rezende = map(data, ~nls_multstart(cal.length~rezende_2019(temp = temp, q10,a,b,c),
                                                     data = .x,
                                                     iter = c(3,3,3,3),
                                                     start_lower = get_start_vals(.x$temp, .x$cal.length, model_name = 'rezende_2019') - 1,
...
for(i in 1:length(df)){
  d_fit_boot_i<-nest(df[[i]], data = c(temp, cal.length)) %>%
  mutate(rezende = map(data, ~nls_multstart(cal.length~rezende_2019(temp = temp, q10,a,b,c),
                                                     data = .x,
                                                     iter = c(3,3,3,3),
                                                     start_lower = get_start_vals(.x$temp, .x$cal.length, model_name = 'rezende_2019') - 1,
...
gb<-na.omit(gb)
gb<-data.frame("temp"=gb$temp,"cal.length"=gb$cal.length,"bin"=gb$bin,"pop"=gb$pop)
gb1<-gb%>%filter(bin==1)
...
wh<-na.omit(wh)
wh<-data.frame("temp"=wh$temp,"cal.length"=wh$cal.length,"bin"=wh$bin,"pop"=wh$pop)
wh1<-wh%>%filter(bin==1)
...
oy<-na.omit(oy)
oy<-data.frame("temp"=oy$temp,"cal.length"=oy$cal.length,"bin"=oy$bin,"pop"=oy$pop)
oy1<-oy%>%filter(bin==1)
...
bf<-na.omit(bf)
bf<-data.frame("temp"=bf$temp,"cal.length"=bf$cal.length,"bin"=bf$bin,"pop"=bf$pop)
bf1<-bf%>%filter(bin==1)
...
fb<-na.omit(fb)
fb<-data.frame("temp"=fb$temp,"cal.length"=fb$cal.length,"bin"=fb$bin,"pop"=fb$pop)
fb1<-fb%>%filter(bin==1)
...
sk<-na.omit(sk)
sk<-data.frame("temp"=sk$temp,"cal.length"=sk$cal.length,"bin"=sk$bin,"pop"=sk$pop)
sk1<-sk%>%filter(bin==1)
...
wp<-na.omit(wp)
wp<-data.frame("temp"=wp$temp,"cal.length"=wp$cal.length,"bin"=wp$bin,"pop"=wp$pop)
wp1<-wp%>%filter(bin==1)
...
hm<-na.omit(hm)
hm<-data.frame("temp"=hm$temp,"cal.length"=hm$cal.length,"bin"=hm$bin,"pop"=hm$pop)
hm1<-hm%>%filter(bin==1)
...
#ggsave("rtpc_facet.tiff",dpi=1200)
...
#write.csv(ci_extra_params_all,"C:/Users/drewv/Documents/UMASS/data/Uro_ch_2/data/ci_extra_params_all.csv",row.names=T)
