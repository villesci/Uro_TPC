...
growth<-read.csv(here::here("data/growth_test.csv"))
summary(growth)
...
#gb
gb<-read.csv(here::here("test_env/gb/GRBGBWQ.csv"))
gb<-gb[,c(3,7)]
...
wh2019<-read.delim(here::here("test_env/wh/bzbm3h2019.txt"),sep = "", dec = ".")
wh2018<-read.delim(here::here("test_env/wh/bzbm3h2018.txt"),sep = "", dec = ".")
wh2017<-read.delim(here::here("test_env/wh/bzbm3h2017.txt"),sep = "", dec = ".")
wh2016<-read.delim(here::here("test_env/wh/bzbm3h2016.txt"),sep = "", dec = ".")
...
#wh<-read.csv(here::here("test_env/wh/WQBMHWQ.csv"))
#wh<-wh[,c(3,7)]
#wh$rdate<-as.POSIXct(wh$DateTimeStamp,tz="", "%m/%d/%Y%H:%M")
#wh<-na.omit(wh)
#wh$WTMP<-wh$Temp
#wh<-wh[,c(3,4)]
#wh$site<-"wh"
#wh<-wh%>%filter(rdate>="2016-01-01"&rdate<="2019-12-31")

#oy
oy<-read.csv(here::here("test_env/oy/oy.csv"))
oy$site<-"oy"
...
#bf
bf2019<-read.delim(here::here("test_env/bf/bftn7h2019.txt"),sep = "", dec = ".")
bf2018<-read.delim(here::here("test_env/bf/bftn7h2018.txt"),sep = "", dec = ".")
bf2017<-read.delim(here::here("test_env/bf/bftn7h2017.txt"),sep = "", dec = ".")
bf2016<-read.delim(here::here("test_env/bf/bftn7h2016.txt"),sep = "", dec = ".")
...
#fb
fb2019<-read.delim(here::here("test_env/fb/chts1h2019.txt"),sep = "", dec = ".")
fb2018<-read.delim(here::here("test_env/fb/chts1h2018.txt"),sep = "", dec = ".")
fb2017<-read.delim(here::here("test_env/fb/chts1h2017.txt"),sep = "", dec = ".")
fb2016<-read.delim(here::here("test_env/fb/chts1h2016.txt"),sep = "", dec = ".")
...
#sk
sk<-read.csv(here::here("test_env/sk/SAPDCWQ.csv"))
sk<-sk[,c(3,7)]
...
#wp
wp<-read.csv(here::here("test_env/wp/wp.csv"))
...
#hm
#2014
wiyot2014<-read.csv(here::here("test_env/hm/wiyot2014.csv"),header=T)
...
#2013
wiyot2013<-read.csv(here::here("test_env/hm/wiyot2013.csv"),header=T)
...
#2012
wiyot2012<-read.csv(here::here("test_env/hm/wiyot2012.csv"),header=T)
...
wiyot2012$site<-"hm"
#2016
wiyot2016<-read.csv(here::here("test_env/hm/wiyot2016.csv"),header=T)
...
q[7,9]<-hm%>%filter(rdate>=as.Date(paste(year(rdate),05,01,sep="-")),rdate<=as.Date(paste(year(rdate),05,30,sep="-")))%>%summarize(Value=mean(WTMP)) #inat
q[6,9]<-wp%>%filter(rdate>=as.Date(paste(year(rdate),05,01,sep="-")),rdate<=as.Date(paste(year(rdate),05,30,sep="-")))%>%summarize(Value=mean(WTMP))#ruesink
#max (spring2)
...
q[7,10]<-hm%>%filter(rdate>=as.Date(paste(year(rdate),06,01,sep="-")),rdate<=as.Date(paste(year(rdate),07,15,sep="-")))%>%summarize(Value=mean(WTMP))#inat
q[6,10]<-wp%>%filter(rdate>=as.Date(paste(year(rdate),06,01,sep="-")),rdate<=as.Date(paste(year(rdate),07,15,sep="-")))%>%summarize(Value=mean(WTMP))#ruesink
...
#ggplot(data=temp,aes(x=rdate,y=WTMP,color=site))+ggplot2::scale_fill_viridis_d(name="site",labels=c("gre#at bay, NH","woods hole, MA","oyster, VA","beaufort, NC","folly beach, SC","skidaway, GA","willapa, #WA","humboldt, CA"),option="D")+scale_color_viridis_d(name="site",labels=c("great bay, NH","woods hole, #MA","oyster, VA","beaufort, NC","folly beach, SC","skidaway, GA","willapa, WA","humboldt, #CA"),option="D")+theme_classic()+geom_vline(xintercept = 152)+geom_vline(xintercept = 273)+ylab("SST #(°C)")+xlab("")+scale_x_date(date_labels = #"%b")+theme(text=element_text(family="sanserif",size=14))+stat_summary(geom="line",fun.y=base::mean,size#=2)+guides(color=guide_legend(override.aes=list(fill=NA)))+geom_rect(data=data_s,aes(xmin=xmin,xmax=xmax#,ymin=ymin,ymax=ymax),fill=("grey"),alpha=0.85,inherit.aes=F)
#
#ggplot(data=temp,aes(x=date,y=WTMP,color=site))+ggplot2::scale_fill_viridis_d(name="Site",labels=c("Grea#t Bay, NH","Woods Hole, MA","Oyster, VA","Beaufort, NC","Folly Beach, SC","Skidaway, GA","Willapa, #WA","Humboldt, CA"),option="D")+scale_color_viridis_d(name="Site",labels=c("Great Bay, NH","Woods Hole, #MA","Oyster, VA","Beaufort, NC","Folly Beach, SC","Skidaway, GA","Willapa, WA","Humboldt, #CA"),option="D")+theme_classic()+geom_vline(xintercept = 152)+geom_vline(xintercept = 273)+ylab("SST #(°C)")+xlab("")+scale_x_date(date_labels = #"%b")+theme(text=element_text(family="sanserif",size=14))+stat_summary(geom="line",fun.y=base::mean,size#=2)+guides(color=guide_legend(override.aes=list(fill=NA)))
#
##north-south  development mean and season length above 10
#ggplot(data=temp,aes(x=date,y=WTMP,color=site))+geom_rect(data=data_s,aes(xmin=xmin,xmax=xmax,ymin=ymin,#ymax=ymax),fill=("grey"),inherit.aes=F)+geom_rect(data=dev_dates,aes(xmin=xmin,xmax=xmax,ymin=ymin,ymax=#ymax,fill=site),alpha=0.2,inherit.aes=F)+scale_fill_manual(name="Site",labels=c("Great Bay, #NH","Skidaway, GA"),values=c("yellow","blue"))+ggplot2::scale_color_viridis_d(name="Site",labels=c("Grea#t Bay, NH","Woods Hole, MA","Oyster, VA","Beaufort, NC","Folly Beach, SC","Skidaway, GA","Willapa, #WA","Humboldt, CA"),option="D")+theme_classic()+geom_vline(xintercept = 152)+geom_vline(xintercept = #273)+ylab("SST (°C)")+xlab("")+scale_x_date(date_labels = #"%b")+theme(text=element_text(family="sanserif",size=14))+stat_summary(geom="line",fun.y=base::mean,size#=2)+guides(color=guide_legend(override.aes=list(fill=NA)))+
#  geom_segment(aes(y=20,yend=20,x=as.Date("2018-03-01",format="%Y-%m-%d")),xend=as.Date("2018-05-30",for#mat="%Y-%m-%d"),size=2,color="black")+geom_segment(aes(y=22.7,yend=22.7,x=as.Date("2018-07-01",format="%#Y-%m-%d")),xend=as.Date("2018-08-31",format="%Y-%m-%d"),size=2,color="black")
#
##note: the geom_seg argument makes this file size HUGE for some reason. 
#
##temp$site<-factor(temp$site,levels=c("gb","wh","nah15","hmi2","oy","fb","GCSK"))
##spring_env_plot<-ggplot(data=temp,aes(x=date,y=WTMP,color=site))+geom_rect(data=dev_dates,aes(xmin=xmin#,xmax=xmax,ymin=ymin,ymax=ymax,fill=site),alpha=0.2,inherit.aes=F,color=c("red","blue"),fill=c("red","bl#ue"))+scale_fill_manual(name="Site",labels=c("Great Bay, NH","Skidaway, #GA"),values=c("yellow","blue"))+ggplot2::scale_color_viridis_d(name="Site",labels=c("Great Bay, #NH","Woods Hole, MA","Oyster, VA","Beaufort, NC","Folly Beach, SC","Skidaway, GA","Willapa, #WA","Humboldt, CA"),option="D")+theme_classic()+geom_vline(xintercept = 152)+geom_vline(xintercept = #273)+ylab("SST (°C)")+xlab("")+scale_x_date(date_labels = #"%b")+theme(text=element_text(family="Times",size=22))+stat_summary(geom="line",fun.y=base::mean,size=2)#+guides(color=guide_legend(override.aes=list(fill=NA)))+geom_segment(aes(y=20,yend=20,x=as.Date("2018-03#-01",format="%Y-%m-%d")),xend=as.Date("2018-05-30",format="%Y-%m-%d"),size=2,color="black")+geom_segment#(aes(y=22.7,yend=22.7,x=as.Date("2018-07-01",format="%Y-%m-%d")),xend=as.Date("2018-08-31",format="%Y-%m#-%d"),size=2,color="black")
#
#for figure
#ggplot(data=temp,aes(x=date,y=WTMP,color=site))+geom_rect(data=dev_dates,aes(xmin=xmin,xmax=xmax,ymin=ym#in,ymax=ymax,fill=site),alpha=0.2,inherit.aes=F,color=c("red","blue"),fill=c("red","blue"))+scale_fill_m#anual(labels=c("Great Bay, NH","Skidaway, #GA"),values=c("yellow","blue"))+ggplot2::scale_color_viridis_d(labels=c("Great Bay, NH","Woods Hole, #MA","Oyster, VA","Beaufort, NC","Folly Beach, SC","Skidaway, GA","Willapa, WA","Humboldt, #CA"),option="D")+theme_classic()+geom_vline(xintercept = 152)+geom_vline(xintercept = 273)+ylab("SST #(°C)")+xlab("")+scale_x_date(date_labels = #"%b")+theme(text=element_text(family="Times",size=22),axis.text=element_text(color="black",size=22),lege#nd.title = element_blank())+stat_summary(geom="line",fun.y=base::mean,size=2)+guides(color=guide_legend(#override.aes=list(fill=NA)))


#season length
gb_length<-gb%>%mutate(day=as.Date(rdate,format="%Y-%m-%d"))%>%group_by(day)%>%dplyr::summarise(daily_wtmp=mean(WTMP))%>%na.omit()%>%filter(daily_wtmp>10)
sk_length<-sk%>%mutate(day=as.Date(rdate,format="%Y-%m-%d"))%>%group_by(day)%>%dplyr::summarise(daily_wtmp=mean(WTMP))%>%na.omit()%>%filter(daily_wtmp>10)
...
#wh
  #wh1
mod_wh1<-nls_multstart(cal.length~boatman_2017(temp=temp,max, tmin, tmax, a, b),data=wh1,iter=c(3,3,3,3,3),
                   start_lower = get_start_vals(wh1$temp, wh1$cal.length, model_name = 'boatman_2017')-1,
                  start_upper = get_start_vals(wh1$temp, wh1$cal.length, model_name = 'boatman_2017')+1,
                   lower = get_lower_lims(wh1$temp, wh1$cal.length, model_name = 'boatman_2017'),
                   upper = get_upper_lims(wh1$temp, wh1$cal.length, model_name = 'boatman_2017'),
                   supp_errors = 'Y',
...
#wh2
mod_wh2<-nls_multstart(cal.length~boatman_2017(temp=temp,max, tmin, tmax, a, b),data=wh2,iter=c(3,3,3,3,3),
                   start_lower = get_start_vals(wh2$temp, wh2$cal.length, model_name = 'boatman_2017')-1,
                  start_upper = get_start_vals(wh2$temp, wh2$cal.length, model_name = 'boatman_2017')+1,
                   lower = get_lower_lims(wh2$temp, wh2$cal.length, model_name = 'boatman_2017'),
                   upper = get_upper_lims(wh2$temp, wh2$cal.length, model_name = 'boatman_2017'),
                   supp_errors = 'Y',
                   convergence_count = FALSE)
...
#wh3
mod_wh3<-nls_multstart(cal.length~boatman_2017(temp=temp,max, tmin, tmax, a, b),data=wh3,iter=c(3,3,3,3,3),
                   start_lower = get_start_vals(wh3$temp, wh3$cal.length, model_name = 'boatman_2017')-1,
                  start_upper = get_start_vals(wh3$temp, wh3$cal.length, model_name = 'boatman_2017')+1,
                   lower = get_lower_lims(wh3$temp, wh3$cal.length, model_name = 'boatman_2017'),
                   upper = get_upper_lims(wh3$temp, wh3$cal.length, model_name = 'boatman_2017'),
                   supp_errors = 'Y',
                   convergence_count = FALSE)
...
#gb
  #gb1
mod_gb1<-nls_multstart(cal.length~boatman_2017(temp=temp,max, tmin, tmax, a, b),data=gb1,iter=c(3,3,3,3,3),
                   start_lower = get_start_vals(gb1$temp, gb1$cal.length, model_name = 'boatman_2017')-1,
                  start_upper = get_start_vals(gb1$temp, gb1$cal.length, model_name = 'boatman_2017')+1,
                   lower = get_lower_lims(gb1$temp, gb1$cal.length, model_name = 'boatman_2017'),
                   upper = get_upper_lims(gb1$temp, gb1$cal.length, model_name = 'boatman_2017'),
                   supp_errors = 'Y',
                   convergence_count = FALSE)
...
#gb2
mod_gb2<-nls_multstart(cal.length~boatman_2017(temp=temp,max, tmin, tmax, a, b),data=gb2,iter=c(3,3,3,3,3),
                   start_lower = get_start_vals(gb2$temp, gb2$cal.length, model_name = 'boatman_2017')-1,
                  start_upper = get_start_vals(gb2$temp, gb2$cal.length, model_name = 'boatman_2017')+1,
                   lower = get_lower_lims(gb2$temp, gb2$cal.length, model_name = 'boatman_2017'),
                   upper = get_upper_lims(gb2$temp, gb2$cal.length, model_name = 'boatman_2017'),
                   supp_errors = 'Y',
                   convergence_count = FALSE)
...
#gb3
mod_gb3<-nls_multstart(cal.length~boatman_2017(temp=temp,max, tmin, tmax, a, b),data=gb3,iter=c(3,3,3,3,3),
                   start_lower = get_start_vals(gb3$temp, gb3$cal.length, model_name = 'boatman_2017')-1,
                  start_upper = get_start_vals(gb3$temp, gb3$cal.length, model_name = 'boatman_2017')+1,
                   lower = get_lower_lims(gb3$temp, gb3$cal.length, model_name = 'boatman_2017'),
                   upper = get_upper_lims(gb3$temp, gb3$cal.length, model_name = 'boatman_2017'),
                   supp_errors = 'Y',
                   convergence_count = FALSE)
...
# oy
#oy1
mod_oy1<-nls_multstart(cal.length~boatman_2017(temp=temp,max, tmin, tmax, a, b),data=oy1,iter=c(3,3,3,3,3),
                   start_lower = get_start_vals(oy1$temp, oy1$cal.length, model_name = 'boatman_2017')-1,
                  start_upper = get_start_vals(oy1$temp, oy1$cal.length, model_name = 'boatman_2017')+1,
                   lower = get_lower_lims(oy1$temp, oy1$cal.length, model_name = 'boatman_2017'),
                   upper = get_upper_lims(oy1$temp, oy1$cal.length, model_name = 'boatman_2017'),
                   supp_errors = 'Y',
                   convergence_count = FALSE)
...
#oy2
mod_oy2<-nls_multstart(cal.length~boatman_2017(temp=temp,max, tmin, tmax, a, b),data=oy2,iter=c(3,3,3,3,3),
                   start_lower = get_start_vals(oy2$temp, oy2$cal.length, model_name = 'boatman_2017')-1,
                  start_upper = get_start_vals(oy2$temp, oy2$cal.length, model_name = 'boatman_2017')+1,
                   lower = get_lower_lims(oy2$temp, oy2$cal.length, model_name = 'boatman_2017'),
                   upper = get_upper_lims(oy2$temp, oy2$cal.length, model_name = 'boatman_2017'),
                   supp_errors = 'Y',
                   convergence_count = FALSE)
...
#oy3
mod_oy3<-nls_multstart(cal.length~boatman_2017(temp=temp,max, tmin, tmax, a, b),data=oy3,iter=c(3,3,3,3,3),
                   start_lower = get_start_vals(oy3$temp, oy3$cal.length, model_name = 'boatman_2017')-1,
                  start_upper = get_start_vals(oy3$temp, oy3$cal.length, model_name = 'boatman_2017')+1,
                   lower = get_lower_lims(oy3$temp, oy3$cal.length, model_name = 'boatman_2017'),
                   upper = get_upper_lims(oy3$temp, oy3$cal.length, model_name = 'boatman_2017'),
                   supp_errors = 'Y',
                   convergence_count = FALSE)
...
# bf
#bf1
mod_bf1<-nls_multstart(cal.length~boatman_2017(temp=temp,max, tmin, tmax, a, b),data=bf1,iter=c(3,3,3,3,3),
                   start_lower = get_start_vals(bf1$temp, bf1$cal.length, model_name = 'boatman_2017')-1,
                  start_upper = get_start_vals(bf1$temp, bf1$cal.length, model_name = 'boatman_2017')+1,
                   lower = get_lower_lims(bf1$temp, bf1$cal.length, model_name = 'boatman_2017'),
                   upper = get_upper_lims(bf1$temp, bf1$cal.length, model_name = 'boatman_2017'),
                   supp_errors = 'Y',
                   convergence_count = FALSE)
...
#bf2
mod_bf2<-nls_multstart(cal.length~boatman_2017(temp=temp,max, tmin, tmax, a, b),data=bf2,iter=c(3,3,3,3,3),
                   start_lower = get_start_vals(bf2$temp, bf2$cal.length, model_name = 'boatman_2017')-1,
                  start_upper = get_start_vals(bf2$temp, bf2$cal.length, model_name = 'boatman_2017')+1,
                   lower = get_lower_lims(bf2$temp, bf2$cal.length, model_name = 'boatman_2017'),
                   upper = get_upper_lims(bf2$temp, bf2$cal.length, model_name = 'boatman_2017'),
                   supp_errors = 'Y',
                   convergence_count = FALSE)
...
#bf3
mod_bf3<-nls_multstart(cal.length~boatman_2017(temp=temp,max, tmin, tmax, a, b),data=bf3,iter=c(3,3,3,3,3),
                   start_lower = get_start_vals(bf3$temp, bf3$cal.length, model_name = 'boatman_2017')-1,
                  start_upper = get_start_vals(bf3$temp, bf3$cal.length, model_name = 'boatman_2017')+1,
                   lower = get_lower_lims(bf3$temp, bf3$cal.length, model_name = 'boatman_2017'),
                   upper = get_upper_lims(bf3$temp, bf3$cal.length, model_name = 'boatman_2017'),
                   supp_errors = 'Y',
                   convergence_count = FALSE)
...
#fb
#fb1
mod_fb1<-nls_multstart(cal.length~boatman_2017(temp=temp,max, tmin, tmax, a, b),data=fb1,iter=c(3,3,3,3,3),
                   start_lower = get_start_vals(fb1$temp, fb1$cal.length, model_name = 'boatman_2017')-1,
                  start_upper = get_start_vals(fb1$temp, fb1$cal.length, model_name = 'boatman_2017')+1,
                   lower = get_lower_lims(fb1$temp, fb1$cal.length, model_name = 'boatman_2017'),
                   upper = get_upper_lims(fb1$temp, fb1$cal.length, model_name = 'boatman_2017'),
                   supp_errors = 'Y',
                   convergence_count = FALSE)
...
#fb2
mod_fb2<-nls_multstart(cal.length~boatman_2017(temp=temp,max, tmin, tmax, a, b),data=fb2,iter=c(3,3,3,3,3),
                   start_lower = get_start_vals(fb2$temp, fb2$cal.length, model_name = 'boatman_2017')-1,
                  start_upper = get_start_vals(fb2$temp, fb2$cal.length, model_name = 'boatman_2017')+1,
                   lower = get_lower_lims(fb2$temp, fb2$cal.length, model_name = 'boatman_2017'),
                   upper = get_upper_lims(fb2$temp, fb2$cal.length, model_name = 'boatman_2017'),
                   supp_errors = 'Y',
                   convergence_count = FALSE)
...
#fb3
mod_fb3<-nls_multstart(cal.length~boatman_2017(temp=temp,max, tmin, tmax, a, b),data=fb3,iter=c(3,3,3,3,3),
                   start_lower = get_start_vals(fb3$temp, fb3$cal.length, model_name = 'boatman_2017')-1,
                  start_upper = get_start_vals(fb3$temp, fb3$cal.length, model_name = 'boatman_2017')+1,
                   lower = get_lower_lims(fb3$temp, fb3$cal.length, model_name = 'boatman_2017'),
                   upper = get_upper_lims(fb3$temp, fb3$cal.length, model_name = 'boatman_2017'),
                   supp_errors = 'Y',
                   convergence_count = FALSE)
...
#sk
#sk1
mod_sk1<-nls_multstart(cal.length~boatman_2017(temp=temp,max, tmin, tmax, a, b),data=sk1,iter=c(3,3,3,3,3),
                   start_lower = get_start_vals(sk1$temp, sk1$cal.length, model_name = 'boatman_2017')-1,
                  start_upper = get_start_vals(sk1$temp, sk1$cal.length, model_name = 'boatman_2017')+1,
                   lower = get_lower_lims(sk1$temp, sk1$cal.length, model_name = 'boatman_2017'),
                   upper = get_upper_lims(sk1$temp, sk1$cal.length, model_name = 'boatman_2017'),
                   supp_errors = 'Y',
                   convergence_count = FALSE)
...
#sk2
mod_sk2<-nls_multstart(cal.length~boatman_2017(temp=temp,max, tmin, tmax, a, b),data=sk2,iter=c(3,3,3,3,3),
                   start_lower = get_start_vals(sk2$temp, sk2$cal.length, model_name = 'boatman_2017')-1,
                  start_upper = get_start_vals(sk2$temp, sk2$cal.length, model_name = 'boatman_2017')+1,
                   lower = get_lower_lims(sk2$temp, sk2$cal.length, model_name = 'boatman_2017'),
                   upper = get_upper_lims(sk2$temp, sk2$cal.length, model_name = 'boatman_2017'),
                   supp_errors = 'Y',
                   convergence_count = FALSE)
...
#sk3
mod_sk3<-nls_multstart(cal.length~boatman_2017(temp=temp,max, tmin, tmax, a, b),data=sk3,iter=c(3,3,3,3,3),
                   start_lower = get_start_vals(sk3$temp, sk3$cal.length, model_name = 'boatman_2017')-1,
                  start_upper = get_start_vals(sk3$temp, sk3$cal.length, model_name = 'boatman_2017')+1,
                   lower = get_lower_lims(sk3$temp, sk3$cal.length, model_name = 'boatman_2017'),
                   upper = get_upper_lims(sk3$temp, sk3$cal.length, model_name = 'boatman_2017'),
                   supp_errors = 'Y',
                   convergence_count = FALSE)
...
#wp
#wp1
mod_wp1<-nls_multstart(cal.length~boatman_2017(temp=temp,max, tmin, tmax, a, b),data=wp1,iter=c(3,3,3,3,3),
                   start_lower = get_start_vals(wp1$temp, wp1$cal.length, model_name = 'boatman_2017')-1,
                  start_upper = get_start_vals(wp1$temp, wp1$cal.length, model_name = 'boatman_2017')+1,
                   lower = get_lower_lims(wp1$temp, wp1$cal.length, model_name = 'boatman_2017'),
                   upper = get_upper_lims(wp1$temp, wp1$cal.length, model_name = 'boatman_2017'),
                   supp_errors = 'Y',
                   convergence_count = FALSE)
...
#wp2
mod_wp2<-nls_multstart(cal.length~boatman_2017(temp=temp,max, tmin, tmax, a, b),data=wp2,iter=c(3,3,3,3,3),
                   start_lower = get_start_vals(wp2$temp, wp2$cal.length, model_name = 'boatman_2017')-1,
                  start_upper = get_start_vals(wp2$temp, wp2$cal.length, model_name = 'boatman_2017')+1,
                   lower = get_lower_lims(wp2$temp, wp2$cal.length, model_name = 'boatman_2017'),
                   upper = get_upper_lims(wp2$temp, wp2$cal.length, model_name = 'boatman_2017'),
                   supp_errors = 'Y',
                   convergence_count = FALSE)
...
#wp3
mod_wp3<-nls_multstart(cal.length~boatman_2017(temp=temp,max, tmin, tmax, a, b),data=wp3,iter=c(3,3,3,3,3),
                   start_lower = get_start_vals(wp3$temp, wp3$cal.length, model_name = 'boatman_2017')-1,
                  start_upper = get_start_vals(wp3$temp, wp3$cal.length, model_name = 'boatman_2017')+1,
                   lower = get_lower_lims(wp3$temp, wp3$cal.length, model_name = 'boatman_2017'),
                   upper = get_upper_lims(wp3$temp, wp3$cal.length, model_name = 'boatman_2017'),
                   supp_errors = 'Y',
                   convergence_count = FALSE)
...
#hm
#hm1
mod_hm1<-nls_multstart(cal.length~boatman_2017(temp=temp,max, tmin, tmax, a, b),data=hm1,iter=c(3,3,3,3,3),
                   start_lower = get_start_vals(hm1$temp, hm1$cal.length, model_name = 'boatman_2017')-1,
                  start_upper = get_start_vals(hm1$temp, hm1$cal.length, model_name = 'boatman_2017')+1,
                   lower = get_lower_lims(hm1$temp, hm1$cal.length, model_name = 'boatman_2017'),
                   upper = get_upper_lims(hm1$temp, hm1$cal.length, model_name = 'boatman_2017'),
                   supp_errors = 'Y',
                   convergence_count = FALSE)
...
#hm2
mod_hm2<-nls_multstart(cal.length~boatman_2017(temp=temp,max, tmin, tmax, a, b),data=hm2,iter=c(3,3,3,3,3),
                   start_lower = get_start_vals(hm2$temp, hm2$cal.length, model_name = 'boatman_2017')-1,
                  start_upper = get_start_vals(hm2$temp, hm2$cal.length, model_name = 'boatman_2017')+1,
                   lower = get_lower_lims(hm2$temp, hm2$cal.length, model_name = 'boatman_2017'),
                   upper = get_upper_lims(hm2$temp, hm2$cal.length, model_name = 'boatman_2017'),
                   supp_errors = 'Y',
                   convergence_count = FALSE)
...
#hm3
mod_hm3<-nls_multstart(cal.length~boatman_2017(temp=temp,max, tmin, tmax, a, b),data=hm3,iter=c(3,3,3,3,3),
                   start_lower = get_start_vals(hm3$temp, hm3$cal.length, model_name = 'boatman_2017')-1,
                  start_upper = get_start_vals(hm3$temp, hm3$cal.length, model_name = 'boatman_2017')+1,
                   lower = get_lower_lims(hm3$temp, hm3$cal.length, model_name = 'boatman_2017'),
                   upper = get_upper_lims(hm3$temp, hm3$cal.length, model_name = 'boatman_2017'),
                   supp_errors = 'Y',
                   convergence_count = FALSE)
...
rmax$site<-rmax$code
rmax<-rmax[,c(3:4,6,10)]
colnames(rmax)<-c("conf_lower_rmax","conf_upper_rmax","estimate_rmax","site")
topt<-filter(ci_extra_params_all,param=="topt")
topt$site<-topt$code
topt<-topt[,c(3:4,6,10)]
colnames(topt)<-c("conf_lower_topt","conf_upper_topt","estimate_topt","site")
brkpts<-merge(brkpts,rmax,by=("site"))
brkpts<-merge(brkpts,topt,by=("site"))

brkpts<-brkpts[,-c(11:14,16:18)]
...
ggplot(brkpts,aes(x=seasonlength10,y=brkpty,label=site))+
  ...
  geom_smooth(method='lm',se=T,color="black")+scale_shape_manual(labels=c("Atlantic","Pacific"),name="Ocean",values=c(16,17))+geom_errorbar(aes(ymin=conf_lower_rmax, ymax=conf_upper_rmax),width=4)+geom_text()
...
ggplot(brkpts,aes(x=spring,y=brkptx,label = site))+
  ...
  scale_color_manual(labels=c("Atlantic","Pacific"),name="Ocean", values=c('red','blue'))+
  scale_fill_manual(labels=c("Atlantic","Pacific"),name="Ocean", values=c('red','blue'))+
  scale_shape_manual(labels=c("Atlantic","Pacific"),name="Ocean",values=c(16,17))+ylab("Thermal Optima")+ylab("Topt (°C)")+geom_errorbar(aes(ymin=conf_lower_topt, ymax=conf_upper_topt), width=.1)+geom_text(check_overlap = TRUE)
...
#brkpts_wh<-filter(brkpts,pop!="Woods Hole") # drops out signficance
...
brkptym_wh<-lm(brkpty~seasonlength10,brkpts_wh)
summary(brkptym_wh)

ggplot(brkpts_wh,aes(x=seasonlength10,y=brkpty))+
...
mods.brky2<-list(
  ...
  "spring"=glm(brkptx~spring,brkpts_wh,family="gaussian"),
  "spring2"=glm(brkptx~spring2,brkpts_wh,family="gaussian")
  )

aictab(mods.brky2)

brkptym2<-lm(brkptx~spring,brkpts_wh)
summary(brkptym2)
