...
uro.con2$rate_mcp<-uro.con2$rate
...
seg.wh<-segmented(m.wh,seg.Z = ~Temp, psi=28)
...
seg.bf<-segmented(m.bf,seg.Z = ~Temp, psi=22,seg.control(maxit.glm=10),fix.npsi=T,n.boot=5000)

m.fb<-glm(rate~Temp,fb,family=gaussian)
seg.fb<-segmented(m.fb,seg.Z = ~Temp, psi=24)

m.sk<-glm(rate~Temp,sk,family=gaussian)
seg.sk<-segmented(m.sk,seg.Z = ~Temp, psi=24,,seg.control(maxit.glm=10),fix.npsi=T,n.boot=50))
...
fitbf_wt=mcp::mcp(model_mcp,data=sk,iter=10000,cores=3,prior=list(cp_1="dunif(22,30)"))
...
brkpts_con<-data.frame(matrix(,nrow=8,ncol=9))
colnames(brkpts_con)<-c("pop","brkptx","brkpty","lat","mean","s.mean","q.mean","t.mean","oce")
...
aictab(mods.brk_con)
summary(glm(brkpty~lat+oce,brkpts_con,family="gaussian"))

ggplot(brkpts_con,aes(x=lat,y=brkpty,color=oce,fill=oce),group=oce)+
...
aictab(mods.brk_con)
summary(glm(brkptx~lat+oce,brkpts_con,family="gaussian"))

ggplot(brkpts_con,aes(x=lat,y=brkptx,color=oce,fill=oce),group=oce)+
  geom_point(size=3)+theme_classic()  
