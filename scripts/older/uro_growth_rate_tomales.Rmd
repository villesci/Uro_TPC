...
alivepoptemp<-growth%>%group_by(temp,pop)%>%dplyr::summarise(alivesum=sum(aliven))
deadpoptemp<-growth%>%group_by(temp,pop)%>%dplyr::summarise(deadsum=sum(dead))
...
comb<-data.frame(mean,p1)

ggplot(growth,aes(x=temp,y=aliven))+geom_jitter()+geom_smooth(data=comb,aes(x=tempt,y=p1),method="glm",method.args=list(family="binomial"),formula=y~x,color="black")
```
...
ggplot(data=temp,aes(x=date,y=WTMP,color=site))+scale_color_viridis_d(name="Site",labels=c("Great Bay, NH","Woods Hole, MA","Oyster, VA","Beaufort, NC","Folly Beach, SC","Skidaway, GA","Willapa, WA","Humboldt, CA"),option="D")+theme_classic()+geom_vline(xintercept = 152)+geom_vline(xintercept = 273)+ylab("SST (°C)")+xlab("")+scale_fill_discrete(name="Site",labels=c("Great Bay, NH","Woods Hole, MA","Oyster, VA","Beaufort, NC","Folly Beach, SC","Skidaway, GA","Willapa, WA","Humboldt, CA"))+scale_x_date(date_labels = "%b")+theme(text=element_text(family="sanserif",size=14))+stat_summary(geom="line",fun.y=base::mean,size=2)+geom_point(alpha=0.0002,color="black",fill="white")+guides(color=guide_legend(override.aes=list(fill=NA)))
...
to<-read.csv(here::here(('data/tomales.cheng.csv')))
#to$cal.length<-to$cal.length*(24/27)

bf$cal.length<-bf$cal.length/24
fb$cal.length<-fb$cal.length/24
sk$cal.length<-sk$cal.length/24
oy$cal.length<-oy$cal.length/24
wh$cal.length<-wh$cal.length/24
gb$cal.length<-gb$cal.length/24
wp$cal.length<-wp$cal.length/24
hm$cal.length<-hm$cal.length/24
to$cal.length<-to$cal.length/27
...
seg.gb<-segmented(m.gb,seg.Z = ~temp, psi=29,seg.control(maxit.glm=100),fix.npsi=T,n.boot=5000)
...
m.to<-glm(cal.length~temp,to,family="gaussian")
seg.to<-segmented(m.to,seg.z=~temp,psi=24)
...
predicted.to<-data.frame(temp=seq(xmin,xmax,length.out=100))
predicted.to$cal.length<-predict(seg.to,predicted.to)
predicted.to$pop<-"to"
predicted.to$oce<-"p"

#combine predicted values into a single dataframe
all.length.pred<-rbind(predicted.gb,predicted.wh,predicted.oy,predicted.bf,predicted.fb,predicted.sk,predicted.wp,predicted.hm,predicted.to)
all.length.pred$population<-all.length.pred$pop
all.length.pred$population<-factor(all.length.pred$population,levels=c("gb","wh","oy","bf","fb","sk","wp","hm","to"))

all.length.pred$pop<-factor(all.length.pred$pop,levels=c("gb","wh","oy","bf","fb","sk","wp","hm","to"))


all.length.pred$pop<-ifelse(all.length.pred$pop=="gb","Great Bay",ifelse(all.length.pred$pop=="wh","Woods Hole",ifelse(all.length.pred$pop=="oy","Oyster",ifelse(all.length.pred$pop=="bf","Beaufort",ifelse(all.length.pred$pop=="fb","Folly Beach",ifelse(all.length.pred$pop=="sk","Skidaway",ifelse(all.length.pred$pop=="wp","Willapa",ifelse(all.length.pred$pop=="hm","Humboldt",ifelse(all.length.pred$pop=="to","Tomales",NA)))))))))

all.length.pred$pop<-as.factor(all.length.pred$pop)
all.length.pred$pop<-factor(all.length.pred$pop,levels=c("Great Bay","Woods Hole","Oyster","Beaufort","Folly Beach","Skidaway","Willapa","Humboldt","Tomales"))
...
growth.alive$pop<-as.factor(growth.alive$pop)
growth.alive$population<-factor(growth.alive$pop,levels=c("Great Bay","Woods Hole","Oyster","Beaufort","Folly Beach","Skidaway","Willapa","Humboldt","Tomales"))
growth.alive$pop<-factor(growth.alive$pop,levels=c("Great Bay","Woods Hole","Oyster","Beaufort","Folly Beach","Skidaway","Willapa","Humboldt","Tomales"))
...
ggplot(all.length.pred,aes(x=temp,y=cal.length))+geom_line(aes(group=population,x=temp,y=cal.length,color=population,linetype=population,size=population))+
  ...
  scale_linetype_manual(values=c(1,1,1,1,1,1,3,3,3),labels=c("Great Bay","Woods Hole","Oyster","Beaufort","Folly Beach","Skidaway","Willapa","Humboldt","Tomales"),name="Population")+
  scale_size_manual(values=c(1.2,1.2,1.2,1.2,1.2,1.2,1.2,1.2,1.2),labels=c("Great Bay","Woods Hole","Oyster","Beaufort","Folly Beach","Skidaway","Willapa","Humboldt","Tomales"),name="Population")+
  scale_color_manual(values=c("dark violet","navy","forest green","gold","dark orange","tomato","dark violet","navy","forest green"),labels=c("Great Bay","Woods Hole","Oyster","Beaufort","Folly Beach","Skidaway","Willapa","Humboldt","Tomales"),name="Population")+theme(text=element_text(family="arial",size=22))

ggplot()+geom_boxplot(data=growth.alive,aes(x=temp,y=cal.length,group=interaction(pop,temp),color=pop,fill=pop),alpha=0.7)+scale_color_manual(values=c("dark violet","navy","forest green","gold","dark orange","tomato","dark violet","navy","forest green"),labels=c("Great Bay","Woods Hole","Oyster","Beaufort","Folly Beach","Skidaway","Willapa","Humboldt","Tomales"),name="Population")+scale_fill_manual(values=c(NA,NA,NA,NA,NA,NA,"gray","gray","gray"),labels=c("Great Bay","Woods Hole","Oyster","Beaufort","Folly Beach","Skidaway","Willapa","Humboldt","Tomales"),name="Population")+new_scale_color()+new_scale_fill()+geom_line(data=all.length.pred,aes(x=temp,y=cal.length,group=population,color=population,linetype=population,size=population))+
  ylab("Shell Length (mm)")+xlab("Common Garden Temperature (°C)")+theme_classic()+
  scale_x_continuous(breaks=c(16,20,24,26,28,30))+
  scale_linetype_manual(values=c(1,1,1,1,1,1,3,3,3),labels=c("Great Bay","Woods Hole","Oyster","Beaufort","Folly Beach","Skidaway","Willapa","Humboldt","Tomales"),name="Population")+
  scale_size_manual(values=c(1.2,1.2,1.2,1.2,1.2,1.2,1.2,1.2,1.2),labels=c("Great Bay","Woods Hole","Oyster","Beaufort","Folly Beach","Skidaway","Willapa","Humboldt","Tomales"),name="Population")+
...
ggplot(all.length.pred,aes(x=temp,y=cal.length))+geom_line(aes(group=population,x=temp,y=cal.length,color=population,linetype=population,size=population))+
  ylab("Shell Length (mm)")+xlab("Common Garden Temperature (°C)")+theme_classic()+
  scale_x_continuous(breaks=c(16,20,24,26,28,30))+
  scale_linetype_manual(values=c(1,1,1,1,1,1,3,3),labels=c("New Hampshire","Massachusetts","Virginia","North Carolina","South Carolina","Georgia","Washington (invasive)","California (invasive"),name="Population")+
  scale_size_manual(values=c(1.2,1.2,1.2,1.2,1.2,1.2,1.2,1.2),labels=c("New Hampshire","Massachusetts","Virginia","North Carolina","South Carolina","Georgia","Washington (invasive)","California (invasive"),name="Population")+
  scale_color_manual(values=c("dark violet","navy","forest green","gold","dark orange","tomato","dark violet","navy"),labels=c("New Hampshire","Massachusetts","Virginia","North Carolina","South Carolina","Georgia","Washington (invasive)","California (invasive"),name="Population")+theme(text=element_text(family="arial",size=22))
...
```{r,include=F,echo=F,warning=F}
brkpts<-data.frame(matrix(,nrow=9,ncol=19))
...
brkpts$pop<-c("Willapa","Humboldt","Great Bay","Woods Hole","Oyster","Beaufort","Folly Beach","Skidaway","Tomales")
...
brkpts[9,3]<-(seg.to$psi[[2]]*coef(seg.to)[[2]])+(coef(seg.to)[[1]])
...
brkpts$lat<-ifelse(brkpts$pop=="Beaufort",34.819,ifelse(brkpts$pop=="Folly Beach",32.660525,
                                                        ifelse(brkpts$pop=="Great Bay",43.089589,ifelse(brkpts$pop=="Humboldt",40.849448,ifelse(brkpts$pop=="Oyster",
                                                                                                                                                37.288562,ifelse(brkpts$pop=="Tomales",38.12805,ifelse(brkpts$pop=="Woods Hole",41.57687,ifelse(brkpts$pop=="Willapa",46.5007,ifelse(brkpts$pop=="Skidaway",31.970,ifelse(brkpts$pop=="Tomales",38.12805
,NA))))))))))
...
brkpts[7,18]<-q[5,7]
brkpts[8,18]<-q[9,7]
brkpts[9,18]<-to3%>%mutate(day=as.Date(rdate,format="%Y-%m-%d"))%>%group_by(day)%>%dplyr::summarise(daily_wtmp=mean(WTMP))%>%na.omit()%>%filter(daily_wtmp>10)%>%tally
...
brkpts[7,19]<-q[5,8]
brkpts[8,19]<-q[9,8]
brkpts[9,19]<-to3%>%mutate(day=as.Date(rdate,format="%Y-%m-%d"))%>%group_by(day)%>%dplyr::summarise(daily_wtmp=mean(WTMP))%>%na.omit()%>%filter(daily_wtmp>12.5)%>%tally
...
mods.brk<-list(
  ...
  "seasonlengtho16"=glm(brkpty~seasonlength12+oce,brkpts,family="gaussian"),
  ...
  "seasonlength12"=glm(brkpty~seasonlength12*oce,brkpts,family="gaussian"),
...
mods.brkx<-list(
  ...
  "seasonlengtho16"=glm(brkptx~seasonlength12+oce,brkpts,family="gaussian"),
  ...
  "seasonlength12"=glm(brkptx~seasonlength12*oce,brkpts,family="gaussian"),
...
Tomales seems to be an outlier for this pattern of decreasing thermal optima with season length. what would happen if we remove it?
...
mods.brkyq<-list(
  ...
  "seasonlengtho16"=glm(brkptyq~seasonlength12+oce,brkpts,family="gaussian"),
  ...
  "seasonlength12"=glm(brkptyq~seasonlength12*oce,brkpts,family="gaussian"),
...
brkptxmq<-glm(brkptyq~mean,brkpts,family="gaussian")
summary(brkptxmq)

ggplot(brkpts,aes(x=mean,y=brkptyq))+
  geom_point(size=3,aes(color=oce))+geom_smooth(method="lm",se=F,color="black")+theme_classic()+
  ylab("Thermal Optima")+
  xlab("Mean (degrees C)")+
  theme(text=element_text(family="arial",size=22))+
...
mods.brkxq<-list(
  ...
  "seasonlengtho16"=glm(brkptxq~seasonlength12+oce,brkpts,family="gaussian"),
  ...
  "seasonlength12"=glm(brkptxq~seasonlength12*oce,brkpts,family="gaussian"),
...
ggplot(brkpts,aes(x=seasonlength10,y=brkptxq))+
  geom_point(size=3,aes(color=oce))+geom_smooth(method="lm",se=F,color="black")+theme_classic()+
