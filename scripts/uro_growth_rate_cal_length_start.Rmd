...
abc<-aov(cal.length.start~pop,data=growth.alive)
TukeyHSD(abc)
...
m.wh<-glm(cal.length~temp+cal.length.start,wh,family=gaussian)
seg.wh<-segmented(m.wh,seg.Z = ~temp, psi=(temp=24),seg.control(maxit.glm=100),fix.npsi=T,n.boot=500)

m.gb<-glm(cal.length~temp+cal.length.start,gb,family=gaussian)
seg.gb<-segmented(m.gb,seg.Z = ~temp+cal.length.start, psi=list(temp=26,cal.length.start=1.5),seg.control(maxit.glm=100),fix.npsi=T,n.boot=500)

m.oy<-glm(cal.length~temp+cal.length.start,oy,family=gaussian)
seg.oy<-segmented(m.oy,seg.Z = ~temp+cal.length.start, psi=list(temp=24,cal.length.start=1.5))

m.bf<-glm(cal.length~temp+cal.length.start,bf,family=gaussian)
seg.bf<-segmented(m.bf,seg.Z = ~temp+cal.length.start, psi=list(temp=24,cal.length.start=1.5))

m.fb<-glm(cal.length~temp+cal.length.start,fb,family=gaussian)
seg.fb<-segmented(m.fb,seg.Z = ~temp+cal.length.start, psi=list(temp=24,cal.length.start=1.5))

m.sk<-glm(cal.length~temp+cal.length.start,sk,family=gaussian)
seg.sk<-segmented(m.sk,seg.Z = ~temp+cal.length.start, psi=list(temp=22,cal.length.start=1.5),seg.control(maxit.glm=100),fix.npsi=T,n.boot=500)

hm<-na.omit(hm)
m.hm<-glm(cal.length~temp+cal.length.start,hm,family=gaussian)
seg.hm<-segmented(m.hm,seg.Z = ~temp+cal.length.start, psi=list(temp=24,cal.length.start=1.2))

m.wp<-glm(cal.length~temp+cal.length.start,wp,family=gaussian)
seg.wp<-segmented(m.wp,seg.Z = ~temp+cal.length.start, psi=list(temp=24,cal.length.start=1.5))
...
zmin<-min(growth.alive$cal.length.start,na.rm=T)
zmax<-max(growth.alive$cal.length.start,na.rm=T)

all.length.seg<-list(seg.gb,seg.wh,seg.oy,seg.bf,seg.fb,seg.sk,seg.wp,seg.hm)


##plotting predicted values
a<-seq(xmin,xmax,length.out=100)
b<-seq(zmin,zmax,length.out=100)
predicted.gb<-data.frame(temp=a,cal.length.start=b)
predicted.gb$cal.length<-predict(seg.gb,predicted.gb)
predicted.gb$pop<-"gb"
predicted.gb$oce<-"a"

predicted.wh<-data.frame(temp=a,cal.length.start=b)
predicted.wh$cal.length<-predict(seg.wh,predicted.wh)
predicted.wh$pop<-"wh"
predicted.wh$oce<-"a"

predicted.oy<-data.frame(temp=a,cal.length.start=b)
predicted.oy$cal.length<-predict(seg.oy,predicted.oy)
predicted.oy$pop<-"oy"
predicted.oy$oce<-"a"

predicted.bf<-data.frame(temp=a,cal.length.start=b)
predicted.bf$cal.length<-predict(seg.bf,predicted.bf)
predicted.bf$pop<-"bf"
predicted.bf$oce<-"a"

predicted.fb<-data.frame(temp=a,cal.length.start=b)
predicted.fb$cal.length<-predict(seg.fb,predicted.fb)
predicted.fb$pop<-"fb"
predicted.fb$oce<-"a"

predicted.sk<-data.frame(temp=a,cal.length.start=b)
predicted.sk$cal.length<-predict(seg.sk,predicted.sk)
predicted.sk$pop<-"sk"
predicted.sk$oce<-"a"

predicted.wp<-data.frame(temp=a,cal.length.start=b)
predicted.wp$cal.length<-predict(seg.wp,predicted.wp)
predicted.wp$pop<-"wp"
predicted.wp$oce<-"p"

predicted.hm<-data.frame(temp=a,cal.length.start=b)
