...
aictab(mods.brk_con)
summary(lm(brkpty~s.mean,brkpts_con))
   
   ggplot(brkpts_con,aes(x=s.mean,y=brkpty))+
    ...
    scale_fill_manual(labels=c("Atlantic","Pacific"),name="Ocean", values=c('red','blue'))+theme(text=element_text(family="arial",size=22))+xlab("Summer Mean Temperature (°C)") 
...
```{r,echo=F,warning=F}
  ...
  "o*max"=glm(brkptx~max*oce,brkpts_con,family="gaussian")
   ...
   aictab(mods.brk_con)
   summary(lm(brkptx~s.mean,brkpts_con))
   
   ggplot(brkpts_con,aes(x=s.mean,y=brkptx))+
...
```{r,echo=F,warning=F}
  ...
  #    theme_classic()+labs(y="Rate of Consumption",x="Temperature (?C)")+facet_wrap(Population~.)+geom_point()
   ...
   ggplot(uro.con3,aes(y=rate,x=Temp,group=Population,linetype=Population))+
      ...
      theme_classic()+labs(y="Log Rate of Consumption",x="Temperature (C)")+theme(text=element_text(family="arial",size=22))
...
```{r,echo=F,warning=F}
  ...
  "o*max"=glm(brkptyq~max*oce,brkpts_con,family="gaussian")
   ...
   brkptxmq<-lm(brkptyq~lat,brkpts_con)
   summary(brkptxmq)
...
```{r,echo=F,warning=F}
  ...
  "o*max"=glm(brkptxq~max*oce,brkpts_con,family="gaussian")
   ...
   brkptxmq<-lm(brkptxq~seasonlength10,brkpts_con)
   summary(brkptxmq)
